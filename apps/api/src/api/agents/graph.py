...
def _finalize_result(result: dict, qdrant_client: QdrantClient) -> dict:
...
def agent_wrapper(question: str, thread_id: str) -> dict:
    ...
    return _finalize_result(result, qdrant_client)
...
def agent_stream_wrapper(question: str, thread_id: str) -> dict:
    ...
    yield _string_for_sse(json.dumps(
        {
            "type": "final_answer",
            "data": _finalize_result(result, qdrant_client)
        }
    ))